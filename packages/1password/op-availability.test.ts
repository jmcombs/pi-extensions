 ...
 * and `1p_run`'s gating: given various `op` probe outcomes and environment, does
 * the code gate on **configuration** (not `op whoami`/`signedIn`)?
 *
...
import factory, { getOpStatus } from "./index.js";
...
function makeApi(): { api: ExtensionAPI; tools: Map<string, StoredTool> } {
  const tools = new Map<string, StoredTool>();
  const noop = (): void => {};
  const api = {
    on: noop as unknown as ExtensionAPI["on"],
    registerTool: ((t: StoredTool) => {
      tools.set(t.name, t);
    ...
    registerFlag: noop as unknown as ExtensionAPI["registerFlag"],
  } as unknown as ExtensionAPI;
  return { api, tools };
}

describe("1p_run gating (ADR 0003)", () => {
  it("attempts the run when configured, even though signedIn is false", async () => {
    const home = await mkdtemp(join(tmpdir(), "1p-run-"));
    const prevHome = process.env.HOME;
    const prevDir = process.env.PI_CODING_AGENT_DIR;
    process.env.HOME = home; // isolate loadShellEnvMap (uses homedir())
    process.env.PI_CODING_AGENT_DIR = home; // isolate warm-on-load / auth.json
    try {
      h.responses = [
        VERSION_OK,
        WHOAMI_FAIL, // cold whoami: "not signed in" — must NOT pre-block
        { match: "account list", stdout: JSON.stringify([{ url: "x.1password.com" }]) },
        { match: "op run --", stdout: "hello-from-op-run" },
      ];
      const { api, tools } = makeApi();
      await factory(api);

      const runTool = tools.get("1p_run");
      expect(runTool?.execute).toBeDefined();

      const res = await runTool?.execute?.("id", { command: "echo hi" });
      const text = (res?.content ?? []).map((c) => c.text).join("\n");

      expect(text).toContain("hello-from-op-run"); // the run was attempted
      expect(text).not.toMatch(/not signed in|not configured/i); // no pre-block
      expect(res?.details?.command).toBe("echo hi");
      expect(h.calls.some((c) => c.includes("op run -- echo hi"))).toBe(true);
    } finally {
