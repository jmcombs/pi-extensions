...
# walk `/context7_setup` and `/headroom_setup` onboarding with the 1Password CLI
# (`op`) genuinely absent. The container has no `op`, no access to the host `~/.pi`,
# and its own throwaway agent dir.
#
...
#             installed globally with Bun (the engine it declares). STOCK — no shim
#             patch. `@jmcombs/pi-1password` now feature-detects `createLocalBash-
#             Operations` (namespace import + `user_bash` hook only when present),
#             so it links under omp's compat shim unmodified.
# - extension: the LOCAL monorepo is COPY'd and `npm ci` symlinks the workspace
#             `@jmcombs/pi-*` packages into node_modules, so both agents load the
#             branch code we are validating — NOT the npm-published packages.
FROM node:22
