...
| D6  | Onboarding = availability-branched | `onboardSecret` first runs an **upfront overwrite gate** (Replace / Keep) when the key already exists, then checks `is1PasswordAvailable()` (= `op` installed **and** an auth path **configured** — service-account token, Connect env, or a desktop/CLI account via `op account list`; **not** gated on `op whoami`/`signedIn`, which false-negatives under the desktop-app biometric integration — see ADR 0003). **Available →** a source menu with three options (UX redesign, ADR 0005): **Find it in 1Password** (the live vault→item→field picker `pickOpReferenceSimple`, with field-step auto-skip for single-credential items) or **Enter a 1Password reference** (validated `op://vault/item/field`) — both stored as `!op read`; or **Paste the key directly** — a **masked** literal entry (D4). **Not available →** the same **masked** literal entry, plus a nudge that installing the 1Password CLI keeps keys in the vault. After writing, a **post-save `verifySecret`** confirms the entry resolves. Existing literal + `!op read` keys already in `auth.json` resolve on read either way (D5). (Maintainer flowchart + ADR 0005; supersedes the earlier "1Password-only" answer.) |
