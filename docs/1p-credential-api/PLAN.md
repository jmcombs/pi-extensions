...
| pi-onboard-offline | human | Real **pi** and real **oh-my-pi** onboarding walked in the op-less interactive container (`docker/run-pi.sh` / `docker/run-ohmypi.sh`): a person completes `/headroom_setup` with `op` absent and eyeballs the masked manual-entry TUI. oh-my-pi loads the LOCAL extension via a container-only exports override (ADR 0008); no product source changed. |
...
| **Real pi + oh-my-pi load the LOCAL extension (no `op`)** (needs: docker-offline) | `docker run --rm pi-ext-interactive:latest` | prints `PI-SMOKE: … setup=ok retrieve=ok session_start=ok local-headroom=/app/packages/headroom… local-1password=/app/packages/1password…` **and** `OHMYPI-SMOKE: … ext-load=ok setup=ok …` — both agents' real loaders register the workspace extension (ADR 0008); oh-my-pi via a container-only exports override that surfaces the real `createLocalBashOperations` |
| Real onboarding walkthrough (no `op`) (needs: pi-onboard-offline) | maintainer runs `docker/run-pi.sh` / `docker/run-ohmypi.sh`, walks `/headroom_setup` with `op` absent | masked manual entry completes; key written to the throwaway `auth.json`; **human-verify** |
